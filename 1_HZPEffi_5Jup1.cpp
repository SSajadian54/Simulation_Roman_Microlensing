...
struct dete{
   double Ne[20][20], Ns[20][20]; 
   double Ml[2][20];
   double Dl[2][20]; 
   double Fl[2][20];
   double Map[2][20]; 
   double Eff[2][2]; ///OHZ,CHZ
};
struct CMD{
...
///===========================================================
    ...
    FILE* fil1;  FILE* fil2;    
    fil1=fopen("./files/HZPpar_5jupiter1.txt","w");
    fil2=fopen("./files/HZPEffi_5jupiter1.txt","w");
    ...
    int typeb=0;
    int flagpw, flagpo;  
    double lml, dd1, dd2, stt, lfl;
 
     double epsilon=0.000000000765425293654; 
     for(int i=0; i<20; ++i) {
     for (int k=0;k<20; ++k) {
     d.Ne[i][k]=0.0;
     d.Ns[i][k]=epsilon;}
     d.Ml[0][i]=d.Dl[0][i]=d.Fl[0][i]=d.Map[0][i]=epsilon; 
     d.Ml[1][i]=d.Dl[1][i]=d.Fl[1][i]=d.Map[1][i]=0.0; } 
     d.Eff[0][0]=d.Eff[0][1]=epsilon; 
     d.Eff[1][0]=d.Eff[1][1]=0.0;  
     ...
     for(int icon=1; icon<2000000000; ++icon){ 
     cout<<"***** Step:  "<<icon<<endl;
...
///************************** OGLE  ********************************************************************
    ...
    if(l.HZP[0]==1)              d.Eff[0][0]+=1.0; ///OHZ
    if(l.HZP[0]==1 and flagpw>0) d.Eff[1][0]+=1.0; ///OHZ
    if(l.HZP[1]==1)              d.Eff[0][1]+=1.0; ///CHZ
    if(l.HZP[1]==1 and flagpw>0) d.Eff[1][1]+=1.0; ///CHZ

    dd1=0.5*sqrt(fabs(pow(10.0,l.logl) ));
    dd2=2.0*sqrt(fabs(pow(10.0,l.logl) ));
    stt=double( (dd2-dd1)/20.0); 
    lml=log10(l.Ml); 
    lfl=log10(fabs(l.Fl[4])); 

    for(int i=0;i<19; ++i){
    ///########################################################################################################
    if(double((l.Dl-i*0.4)*(l.Dl-(i+1)*0.4))<0.0 or l.Dl==double(i*0.4) ){ d.Dl[0][i]+=1.0;/// [0.0,8.0KPC] 0.4
    if(flagpw>0)                                          d.Dl[1][i]+=1.0;}
    ///#######################################################################################################
    if(double((lml-double(-1.1+i*0.09))*(lml-double(-1.1+(i+1)*0.09)))<0.0 or lml==double(-1.1+i*0.09)){d.Ml[0][i]+=1.0;//[-1.1,5Msun]
    if(flagpw>0)                                                                                d.Ml[1][i]+=1.0; 
    for(int j=0;j<19; ++j){
    if(double((l.semi-double(dd1+j*stt))*(l.semi-double(dd1+(j+1)*stt)))<0.0 or l.semi==double(dd1+j*stt)){d.Ns[i][j]+=1.0; 
    if(flagpw>0)                                                                                   d.Ne[i][j]+=1.0;
    break;}}}
    ///#######################################################################################################
    if(((lfl-double(-2.0+i*0.1))*(lfl-double(-2.0+(i+1)*0.1)))<0.0 or lfl==double(-2.0+i*0.1)){ d.Fl[0][i] +=1.0;///[-2.0 ,0.0], 0.1
    if(flagpw>0)                                                                                d.Fl[1][i] +=1.0;}
    ///########################################################################################################   
    if((s.magb[4]-double(15.0+i*0.5))*(s.magb[4]-double(15.0+(i+1)*0.5))<0.0 or s.magb[4]==double(15.0+i*0.5)){d.Map[0][i]+=1.0;
    if(flagpw>0)                                                                                               d.Map[1][i]+=1.0;}}   
    ///########################################################################################################   
    cout<<"Calculating the efficiency *******************"<<endl;
    
    fil2=fopen("./files/HZPEffi_5jupiter1.txt","w");
    fprintf(fil2, "%.8lf    %.8lf  \n", double(d.Eff[1][0]*100.0/d.Eff[0][0]), double(d.Eff[1][1]*100.0/d.Eff[0][1]) ); 
    for(int i=0; i<19; ++i) fprintf(fil2,"%.8lf ",double(d.Dl[1][i]*100.0/d.Dl[0][i]) );
    fprintf(fil2, "\n ************************************************ \n"); 
    for(int i=0; i<19; ++i) fprintf(fil2,"%.8lf ",double(d.Ml[1][i]*100.0/d.Ml[0][i]) );
    fprintf(fil2, "\n ************************************************ \n");
    for(int i=0; i<19; ++i) fprintf(fil2,"%.8lf ",double(d.Fl[1][i]*100.0/d.Fl[0][i]) );
    fprintf(fil2, "\n ************************************************ \n"); 
    for(int i=0; i<19; ++i) fprintf(fil2,"%.8lf ",double(d.Map[1][i]*100.0/d.Map[0][i]));
    fprintf(fil2, "\n ************************************************ \n"); 
    for(int j=0;j<19; ++j){
    for(int i=0;i<19; ++i) fprintf(fil2,"%.8lf ",double(d.Ne[i][j]*100.0/d.Ns[i][j]) );
    fprintf(fil2, "\n");} 
    fprintf(fil2, "\n ************************************************ \n"); 
    fclose(fil2);
    
   fil1=fopen("./files/HZPpar_5jupiter1.txt","a+");
   fprintf(fil1,
   "%d %.3lf  %.3lf  " ///3
...
///**********************************************************************
    ...
    l.Mp=5.0*Mjupiter/Msun;  
    l.q= double(l.Mp/l.Ml);
    l.xls=l.Dl/s.Ds;
    ...
    double fac= sqrt(pow(10.0,l.logl)); 
    l.semi= RandR(0.5*fac, 2.0*fac);
    ...
    double dhz1= 0.95*fac; ///OHZ
    double dhz2= 1.69*fac;
    ...
    for(int i=0; i<M; ++i){
    if(fabs((l.RE/l.Dl/KP)*180.0*3600.0/M_PI)<fabs(FWHM[i]/2.0)) l.theta[i]=1.0;
    else l.theta[i]=0.0;
    
    //cout<<"theta_E (arcs):  "<<double(l.RE*180.0*3600.0/(l.Dl*KP*M_PI) )<<endl;
    //cout<<"RE(meter):  "<<l.RE<<"\t Dl(KP):  "<<l.Dl<<"\t KP:  "<<KP<<"\t M_PI:  "<<M_PI<<endl;
    //cout<<"FWHM:  "<<FWHM[i]<<"\t l.theta[i]:  "<<l.theta[i]<<endl; 
    //int uue; cin>>uue;  
